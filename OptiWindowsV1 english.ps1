############  Arguments to copy into Powershell ISE before starting the script ################
############  Script coded by Metaljisawa. Copyright ################
# --> Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass #

# Tweaks
# Adds a # to disable functions
$tweaks = @(
    # We take admin privilege #
    "RequireAdmin",
    ...
    # Rights / Firewall / Autorun et cie #
    ...
    "DisableUpdateDriver",        	# "EnableUpdateDriver",  # This line disables the Drivers part of WinUpdate (Used to reinstall its drivers properly)
    ...
    "DisableFileDeleteConfirm",    	# "EnableFileDeleteConfirm", ## This line will delete without confirmation the files you delete without going through the trash can
    "HideTaskbarSearchBox",         # "ShowTaskbarSearchBox",
    # "HideTaskView",               # "ShowTaskView",
    # "ShowSmallTaskbarIcons",      # "ShowLargeTaskbarIcons",
    "HideTaskbarTitles",            # "ShowTaskbarTitles",HideTaskbarTitles
    ...
    "HideThisPCFromDesktop",        # "ShowThisPCOnDesktop",HideThisPCFromDesktop
    ...
    "SetVisualFXPerformance",       # "SetVisualFXAppearance",  ## This line is used to change the whole visual of the Windows desktop (so choose well)
    # "DisableThumbnails",          # "EnableThumbnails",
    "DisableThumbsDB",              # "EnableThumbsDB",
    "RemoveENKeyboard",             # "AddENKeyboard",	## This line will remove the English keyboard from your PC (for AZERTY users or other layout)
    ...
    "DisableXboxFeatures",          # "EnableXboxFeatures", ## Here we disable ALL Xbox, if you play games that need it, add a #
    "DisableAdobeFlash",            # "EnableAdobeFlash",
    "UninstallMediaPlayer",         # "InstallMediaPlayer", ## Remember to set VLC by default (Downloaded via this script)
   ...
   # Services - Ninite - WindowsSearch #
    "DisableExtraServices",
    "DoNiniteInstall", 
    "DeleteTempFiles",
    "CleanWinSXS", 
    # "DownloadShutup10",
    "RemoveUnneededComponents", ## We delete the unconnected components to manage the InputLag
    "EnableWindowsSearch",         # "DisableWindowsSearch",
    "DisableCompatibilityAppraiser",
   
    ### Connexion / Windows / GPDW ###
    "DisableConnectedStandby",
    "DisableHibernation",
    "DisableUpdateDriver", # Disables automatic driver updates
...
# Ninite
# Add a # in front of the apps you don't want to install.
# If you want to install an application, just delete the #
## Warning -> If you modify the list by deleting a #, be careful that the last application in the list doesn't have a comma after it! ##
### Check the current script syntax to make sure ###
$niniteapps = @(
    ".net4.7",
    # "7zip",
    # "adaware",
...
Function DisableExtraServices {
    ...
    foreach ($service in $services) {
        if (Get-Service $service -ErrorAction SilentlyContinue)
        {
            ...
            Write-Host " On ignore $service (Le service n existe pas)"
        }
    }
}
...
Function DoNiniteInstall {
    Write-Host "Telechargement de Ninite ..."
    ...
    Write-Host
    Read-Host "Appuies sur ENTER quand Ninite a finit les installations"
}
...
Function RemoveUnneededComponents {
    ...
    foreach ($component in $components) {
        Write-Host "Suppression de : $component"
...
Function DisableGPDWinServices {
    Write-Host "Desactivation des services externes (GPD Win)..."
    ...
    foreach ($service in $services) {
        if (Get-Service $service -ErrorAction SilentlyContinue)
        {
            Write-Host "Arret et desactivation de $service"
            Stop-Service -Name $service
            Get-Service -Name $service | Set-Service -StartupType Disabled
        } else {
...
Function WaitForKey {
    ...
    Read-Host "Appuies sur Entree pour rebooter instantanement ton PC et finir le script. Enjoy !"
}
 
# Restart computer
Function Restart {
    Write-Host "Reboot du Pc en cours..."
    Restart-Computer
}
...
Write-Host
Write-Host
Write-Host " Optimisations W10 adapt� par Metaljisawa"
Write-Host "(Version adapt� de Disassembler)"
Write-Host "Prends bien le temps d'adapter le script a tes envies"
Write-Host
$confirmation = Read-Host "Une fois le script adapte, appuyes sur o et ENTREE !"
 
if ($confirmation -ne 'o') {
    Write-Host
    Write-Host "Le lancement est annule"
    exit
}
 
Write-Host
Write-Host "C est parti !"
